...
import { ArrowDownLeft, Clock, CheckCircle, XCircle } from "lucide-react";
...
export default function DepositRecords() {
  ...
  const { data, isLoading, error } = useQuery({
    ...
    queryFn: async () => {
      // Regular user queries their own deposits using type=deposit
      const res = await apiRequest(`/wallets/transactions?type=deposit`);
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-xs text-slate-400 mt-1">Review all your personal blockchain deposit records.</p>
          </div>
        </div>

        {isLoading ? (
          <div className="flex items-center justify-center p-12 glass-panel bg-slate-900/40">
            <span className="text-slate-400 text-sm animate-pulse">Loading deposit records...</span>
          </div>
        ) : error ? (
          <div className="p-6 glass-panel border border-red-500/20 bg-slate-900/40 text-center">
            <p className="text-red-400 font-semibold text-sm">Failed to retrieve deposit records.</p>
          ...
          <div className="p-12 glass-panel text-center bg-slate-900/40 border border-slate-800">
            <ArrowDownLeft className="h-10 w-10 text-slate-500 mx-auto mb-3 animate-pulse" />
            <p className="text-slate-300 font-medium">No deposits found</p>
            <p className="text-slate-500 text-xs mt-1">Submit a deposit request inside the Wallet system.</p>
          </div>
        ) : (
              ...
              <table className="w-full text-left border-collapse text-xs">
                <thead>
                  <tr className="bg-slate-950/40 text-slate-300 border-b border-slate-800/80 font-medium">
                    ...
                    <th className="px-6 py-4">TX Hash / Notes</th>
                  </tr>
                </thead>
                  ...
                  {transactions.map((tx) => (
                    <tr key={tx._id} className="hover:bg-slate-800/10 transition-colors">
                      ...
                      <td className="px-6 py-4 text-slate-400 max-w-xs truncate">
                        {tx.metadata?.txHash ? (
                          <span className="font-mono">{tx.metadata.txHash}</span>
